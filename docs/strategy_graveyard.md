...
| REGIME_GATE_ENABLED (bb_kdj) | false | REGIME_SKIP_LABELS=choppy,risk_off → disabled | 2026-07-26 batch validation (`scripts/validate_regime.py`, OOS 2024+, N=618 trading days, 356 bb_kdj trades): label distribution: neutral=315d, trending_up=143d, choppy=105d, risk_off=54d, trending_down=1d. Per-label avg PF: trending_up=0.513 (worst), risk_off=0.743, neutral=0.880, choppy=0.928 (best). The original skip labels (choppy,risk_off) are backwards for mean reversion — choppy/ranging days are WHERE bb_kdj edge lives; trending days are when it fails. Current gate was blocking the good days and keeping the bad ones. Gate disabled (REGIME_GATE_ENABLED=false). **Re-enable gate with REGIME_SKIP_LABELS=trending_up,trending_down once sample size grows** (current N is too small for statistical confidence — per-day PF from avg 0.6 trades/day). Validation saves logs/regime_YYYY-MM-DD.json for all 618 classified dates; re-run with `--from-cache` to avoid re-scoring. |
